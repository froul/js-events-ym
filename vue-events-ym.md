...
> 📋 **Нам нужен:** скриншот или HTML страницы бронирования (куда попадает пользователь после «Добавить в корзину»). После этого мы уточним рекомендации для этого шага.

### 2.1 Открытие формы бронирования
...
- CloudPayments уже подключён на странице (подтверждено HTML-источником)
...
> ⚠️ **ВАЖНО:** событие `PURCHASE` и `ecommerce.purchase` должны вызываться строго один раз на заказ. Если есть риск повторного вызова (например, при обновлении страницы) — добавьте флаг в `sessionStorage`.
